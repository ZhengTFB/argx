...
**阶段三：Demo 与 SDK** — 状态：**待审查**

三件产出物都已完成并跑通：`sdk/`（零依赖 SDK + AGENTS.md + README）、`demo/`（示例作品）、
`console/src/views/basic/`（小白控制台）。打开 `localhost:5173` 默认进小白控制台，
点进 ARG 库就能玩一遍示例作品，右边那栏实时显示装置被剧情驱动的过程。等审查确认。
...
| 二 | `docs/ARGX-02-控制台与模拟器.md` | 已通过 |
| 三 | `docs/ARGX-03-Demo与SDK.md` | 待审查 |
...
| 要求 | 结果 | 说明 |
|---|---|---|
| 打开默认进小白控制台，右上角一个入口切到专业版 | ✅ | 不带 hash 就是小白版；右上角「专业控制台」。旧链接（`#simulator` 等）仍然是专业版 |
| 白色 / 浅色配色，简洁直观 | ✅ | 加了一套浅色变量，只给小白版；冒烟里断言了底是浅色、专业版底仍是深色 |
| 专业版保持现状，不要改它 | ✅ | 只把 `App.tsx` 主体整体搬成 `views/pro/ProApp.tsx`，一行没改；专业版冒烟 26 项回归全绿 |
| 一个滑块、一个专业术语都不出现 | ✅ | 界面上没有任何 `input[type=range]`（冒烟有断言），文案里不出现 seq/TTL/抢占/优先级 |
| 引导仅首次显示，之后通过"帮助"再调出 | ✅ | 首次显示 +「知道了，不再显示」写 localStorage；「帮助」随时调出 |
| ARG 库：剧本 / 集成案例、人话说明、需要哪些硬件 | ✅ | 数据仍来自 `works.ts`，剧本来自 `demo/script.json`（同一份，不重写） |
| 设备选择：真实硬件 / 模拟器，两者都能选 | ✅ | 两张卡片，都是正选；模拟器进来就自动连上 |
| 右侧固定一栏：四路当前状态，只读 | ✅ | 数据全部来自 `ARGX.state()`（协议 query/state 回查），界面不另算 |
| 自检：四路依次各跑一遍，每个不到 1 秒，走完约 4 秒 | ✅ | 每路 亮 900ms → 回查 → 熄灭 → 停 450ms；实测 3.6 秒 |
| 三态：绿勾 / 黄叹号 / 转圈 | ✅ | 另加了一个灰圈表示"还没测"——只有三态的话，第一次打开时会误以为"已经测过了" |
| 判断依据是状态回查 | ✅ | 每条都是 发 cue → `ARGX.state()` → 比对；没应答就是黄叹号 |
| 目录结构容纳两套界面，路由现在就留好 | ✅ | `views/basic/` 与 `views/pro/`；两套各占自己的 hash 空间，互不干扰 |
...
3. **专业版与小白版各有一台虚拟装置**（两条独立连接）。同屏比对两边数字时会不一致——
   这是设计结果不是 bug，但第一次看到会愣一下。
4. **`file://` 下 Demo 读不到剧本**（`fetch` 被浏览器挡住）。
   页面会给一句可照做的话，但剧本本身加载不出来——这是浏览器规矩，绕不过去。
5. **小白版不显示故障与协议细节**，这是刻意的（它不是调试台）。
   想制造丢包、看每一帧，去专业版。
