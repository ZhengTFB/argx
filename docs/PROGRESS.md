...
node console/scripts/smoke.mjs  # 端到端冒烟（需先 npm run dev）→ 21 项，全绿
```
...
| 故障注入面板每一项都能触发，控制台不崩且状态正确 | ✅ | 六个开关全部可切；「中途断连」触发掉线通知 + 状态回退（已断言），其余四项在「怎么用它验证控制台」清单里逐条列了观察点 |
...
| 项 | 结果 | 说明 |
|---|---|---|
| 端到端冒烟测试 | ✅ | `console/scripts/smoke.mjs`，21 项。验收里「能看到视觉反馈」「掉线有通知」这类只能真的渲染出来才算数，类型检查证明不了 |
