...
> ⚠️ **仓库数后来变成了三个。** 阶段五按本节方案建了 `argx` 与 `argx-esp32`；
> 阶段六又建了第三个仓库 **`argx-skill`**（<https://github.com/ZhengTFB/argx-skill>，
> 给 ARG 网页作者的 AI 助手用的接入技能包，已建好并推送），
> 见 `docs/ARGX-06-文档与AI-Skill.md` 第 4 节。
> **本节描述的「两个仓库」是阶段五当时的范围，内容仍然有效，不是错误。**
...
### 8.1 实际发生的例外（阶段五收尾时事后追认）

上面那条规则在阶段五被**破了 6 处**，全部是为了让「别人 clone 下来真的能跑」这条硬指标
成立——**不修就是 Pages 上的控制台白屏**。事后追认如下，读到这里的 AI 不必再怀疑是违规：

| 文件 | 改动 | 属于哪一类 |
|---|---|---|
| `console/src/core/sdk.ts` | UMD 全局取不到时退回模块导出 | **不改则白屏** |
| `console/src/core/virtualDevice.ts` | 同上 | **不改则白屏** |
| `console/src/sections/Play.tsx` | `'../demo/'` → `'./demo/'` | **不改则 Demo 播放器 404** |
| `console/scripts/smoke.mjs` | 一条断言的期望路径跟着改（期望值本身错了，不是放宽） | 断言修正 |
| `tests/demo_smoke.mjs` | 浏览器查找改成候选列表逐个试（**断言内容一条没动**） | 可移植性 |
| `console/scripts/lib/browser.mjs` | `launch()` 真的逐个试候选（原来只用第 0 条） | 可移植性 |

后两条不改，则那两个脚本**只在作者机器上能跑**：CI（ubuntu）直接 ENOENT，别人的 Mac 同样。

除此之外，冻结清单里的东西**一个字没动**：`protocol/PROTOCOL.md` 只加了回指 `argx-esp32`
的一行，`device/virtual_device.js` 与 `firmware/argx_mvp/` 只加了互指注释，
`tests/` 的断言内容一条没改，`design/` 只改了 2 处占位 href。

> **教训（下一版任务书该写进去）**：「只许加文件、只许加注释」这条规则，
> 漏掉了**「构建产物与 dev server 不是一回事」**这一类必改项。
> 写死「只许加」的时候，应该同时给出一条出口：
> 「凡测试与闸门全都打 dev server、而发布的是 dist，则构建产物的问题必须在发布前
> 用真浏览器验一遍——这一类修复不受本清单约束，但必须逐条报备」。详见第 5 节。
